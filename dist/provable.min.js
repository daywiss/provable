...
(function (Buffer){
function incr32(e){for(var c,r=e.length;r--;){if(255!==(c=e.readUInt8(r))){c++,e.writeUInt8(c,r);break}e.writeUInt8(0,r)}}function getBlock(e){var c=e._cipher.encryptBlockRaw(e._prev);return incr32(e._prev),c}var xor=require("buffer-xor"),blockSize=16;exports.encrypt=function(e,c){var r=Math.ceil(c.length/blockSize),t=e._cache.length;e._cache=Buffer.concat([e._cache,Buffer.allocUnsafe(r*blockSize)]);for(var n=0;n<r;n++){var a=getBlock(e),i=t+n*blockSize;e._cache.writeUInt32BE(a[0],i+0),e._cache.writeUInt32BE(a[1],i+4),e._cache.writeUInt32BE(a[2],i+8),e._cache.writeUInt32BE(a[3],i+12)}var o=e._cache.slice(0,c.length);return e._cache=e._cache.slice(c.length),xor(c,o)};
...
function cloneMap(r,a,e){var o=a?e(mapToArray(r),CLONE_DEEP_FLAG):mapToArray(r);return arrayReduce(o,addMapEntry,new r.constructor)}var addMapEntry=require("./_addMapEntry"),arrayReduce=require("./_arrayReduce"),mapToArray=require("./_mapToArray"),CLONE_DEEP_FLAG=1;module.exports=cloneMap;
...
function cloneSet(r,e,t){var a=e?t(setToArray(r),CLONE_DEEP_FLAG):setToArray(r);return arrayReduce(a,addSetEntry,new r.constructor)}var addSetEntry=require("./_addSetEntry"),arrayReduce=require("./_arrayReduce"),setToArray=require("./_setToArray"),CLONE_DEEP_FLAG=1;module.exports=cloneSet;
...
function customDefaultsAssignIn(o,t,e,r){return void 0===o||eq(o,objectProto[e])&&!hasOwnProperty.call(r,e)?t:o}var eq=require("./eq"),objectProto=Object.prototype,hasOwnProperty=objectProto.hasOwnProperty;module.exports=customDefaultsAssignIn;
...
function clone(e){return baseClone(e,CLONE_SYMBOLS_FLAG)}var baseClone=require("./_baseClone"),CLONE_SYMBOLS_FLAG=4;module.exports=clone;
...
var apply=require("./_apply"),assignInWith=require("./assignInWith"),baseRest=require("./_baseRest"),customDefaultsAssignIn=require("./_customDefaultsAssignIn"),defaults=baseRest(function(s){return s.push(void 0,customDefaultsAssignIn),apply(assignInWith,void 0,s)});module.exports=defaults;
...
function Engine(e,n){function t(e){return defaults({},e,{id:utils.createSeed(),index:0,count:1,seed:utils.createSeed(),publicSeed:null,maxHex:8})}function u(e){n(e)}function i(){return utils.toInt(i.next(r.publicSeed),r.maxHex)}var r={},s=null;return i.state=function(){return r},i.hashes=function(e,n,t,u){e=e||0,n=t?n||r.count+1:n||r.count;var i=s;return t&&(i=s.concat([r.seed])),i=i.slice(e,n),u?i.reverse():i},i.last=function(e){return i.peek(r.index-1,e)},i.peek=function(e,n){return 0!==e&&null==e&&(e=r.index),n=n||r.publicSeed,utils.rehash(crypto,s[e],n)},i.next=function(e){var n=i.peek(null,e);return assert(n,"end of hash series"),r.index++,u(r),n},r=t(e),r.maxInt=Math.pow(2,4*r.maxHex),assert(r.seed,"requires seed value"),isFunction(n)||(n=function(){}),s=utils.generate(crypto,r.count,r.seed),u(r),i}var crypto=require("crypto"),assert=require("assert"),defaults=require("lodash/defaults"),isFunction=require("lodash/isFunction"),clone=require("lodash/clone"),utils=require("./utils");Engine.generate=utils.generate.bind(null,crypto),Engine.createSeed=utils.createSeed.bind(null,crypto),Engine.rehash=utils.rehash.bind(null,crypto),Engine.sha256=utils.sha256.bind(null,crypto),Engine.toInt=utils.toInt,Engine.toFloat=utils.toFloat,Engine.toBool=utils.toBool,module.exports=Engine;
...
function calcMaxInt(t){return(t=t||defaultHex)==defaultHex?defaultMaxInt:Math.pow(2,4*t)}var assert=require("assert");exports.createSeed=function(t,e){var r="";t=t||32,e=e||"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghipqrstuvwxyz0123456789";for(var a=0;a<t;a++)r+=e.charAt(Math.floor(Math.random()*e.length));return r},exports.sha256=function(t,e){assert(t,"requires crypto");try{return t.createHash("sha256").update(e).digest("hex")}catch(r){return t.SHA256(e).toString()}},exports.rehash=function(t,e,r){if(assert(t,"requires crypto"),assert(e,"requires hash"),null==r)return e;try{return t.createHmac("sha256",e).update(r).digest("hex")}catch(a){return t.algo.HMAC.create(t.algo.SHA256,e).finalize(r).toString()}};var defaultHex=8,defaultMaxInt=Math.pow(2,32);exports.toInt=function(t,e,r){return assert(t,"requires hash"),e=e||defaultHex,assert(t.length>=e,"hash is not long enough"),r?parseInt(t.slice(e),16):parseInt(t.slice(-e),16)},exports.toFloat=function(t,e,r,a,s,n){assert(t,"requires hash"),null==e&&(e=0),null==r&&(r=1);var o=calcMaxInt(s);return e+(r-e)*(exports.toInt(t,s,n)/(a?o+1:o))},exports.toBool=function(t,e,r,a){return null==e&&(e=.5),exports.toFloat(t,0,1,!0,r,a)<e},exports.generate=function(t,e,r){assert(r,"requires seed value"),e=e||1;for(var a=Array(e),s=e-1;s>=0;s--)r=exports.sha256(t,r),a[s]=r;return a};
